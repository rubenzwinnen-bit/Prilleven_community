...
- ✅ **Brok I.3** — styles + cache-buster + docs. Subtle differentiation in styles.css (`.eh-reminder-item-suggestion` met andere achtergrond). Cache-buster `v=2.9.0 → v=2.19.0` in alle files. Docs gesynced in `js/CLAUDE.md` (eersteHapjesSuggestions toegevoegd, eersteHapjes-beschrijving uitgebreid).
...
- ✅ **Brok J.3** — Styles in `styles.css` (filter-toggle pill + recipe-alt grid). Cache-buster `v=2.10.0 → v=2.19.0` in alle files. Docs gesynced in `js/CLAUDE.md`.
...
- **Cache-buster** `v=2.11.0 → v=2.19.0` in alle files.
