// PATCH  /api/eerste-hapjes/meals/[id]   → wijzigen
// DELETE /api/eerste-hapjes/meals/[id]   → verwijderen
//
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'PATCH') {
      ...
      let updates;
      try {
        updates = sanitizeMealPatch(body);
      } catch (err) {
        if (err instanceof HttpError) return json(res, err.status, { error: err.message });
        throw err;
      }
      try {
        const meal = await updateMealLog(auth.userId, id, updates);
    ...
    if (req.method === 'DELETE') {
      try {
        await deleteMealLog(auth.userId, id);
    ...
    console.error('[eerste-hapjes/meals/[id]]', err);
