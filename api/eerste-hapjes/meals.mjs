// GET   /api/eerste-hapjes/meals?child_id=...&from=...&to=...   → maaltijd-logs
// POST  /api/eerste-hapjes/meals                                → nieuw log
//
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      try {
        const meals = await loadMealsForChild(auth.userId, childId, { from, to });
        return json(res, 200, { meals });
      } catch (err) {
    ...
    if (req.method === 'POST') {
      ...
      let clean;
      try {
        clean = sanitizeMealInput(body);
      } catch (err) {
        if (err instanceof HttpError) return json(res, err.status, { error: err.message });
        throw err;
      }
      try {
        const meal = await createMealLog(auth.userId, clean);
        return json(res, 201, { meal });
      } catch (err) {
        if (err instanceof HttpError) return json(res, err.status, { error: err.message });
        throw err;
      }
    ...
    console.error('[eerste-hapjes/meals]', err);
