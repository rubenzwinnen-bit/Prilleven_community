...
| `clients.mjs` | Singleton `supabase` (service-role, `persistSession: false`), `anthropic`, `VOYAGE_API_KEY`, `ANTHROPIC_VIA_GATEWAY`-flag, `anthropicModel(id)`-helper. Anthropic-client routeert via Vercel AI Gateway als `AI_GATEWAY_API_KEY` env-var aanwezig is — baseURL = `https://ai-gateway.vercel.sh` (NIET `/v1/anthropic`; SDK voegt `/v1/messages` zelf toe). Gateway eist provider/model-syntax (`anthropic/claude-haiku-4-5`); gebruik **altijd** `anthropicModel(id)` om elke `model:`-waarde te wrappen voor Anthropic-calls — direct API mode laat de prefix weg. Toegepast in `chat.mjs` (vision + main response), `_lib/conversation.mjs` (titel-Haiku) en `_lib/user-memory.mjs` (fact-extractie). Voyage blijft direct (geen native Gateway-support). |
