...
- **Cooked it / Pril Ritme is voorlopig een frontend-preview:** gekookte slots staan per gebruiker en kalenderweek in `localStorage` (`prilleven_cooked_meals_<user>`). `getCookingMilestoneProgress()` leidt daar lokaal drie zachte momenten uit af (eerste Cooked it, eerste volle ritmeweek, vijf verschillende recepten); na koken verschijnt contextueel een beoordelingsuitnodiging. Drie unieke kookdagen vullen het weekritme en meerdere gerechten op dezelfde dag tellen één keer. Nog niet geschikt als definitieve cross-device opslag.
