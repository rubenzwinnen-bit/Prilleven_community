...
- **Cooked it / Pril Ritme is voorlopig een frontend-preview:** gekookte slots staan per gebruiker en kalenderweek in `localStorage` (`prilleven_cooked_meals_<user>`). Cooked it leidt daar lokaal drie contextuele zachte momenten uit af (eerste Cooked it, eerste volle ritmeweek, vijf verschillende recepten); na koken verschijnt ook een beoordelingsuitnodiging. Dezelfde groene Pril Ritme-tegel bevat de drie weekvinkjes én een rollende historiek van huidig + drie vorige weken (`getCookingRhythmHistory()`); toon geen aparte tegel met een volgend doel en nooit een rode/mislukte weekstatus. Meerdere gerechten op dezelfde dag tellen één keer. Nog niet geschikt als definitieve cross-device opslag.
