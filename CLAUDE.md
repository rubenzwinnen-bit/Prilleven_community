...
/scripts                   → utility scripts (geen prod code)
  /eval                    → HapjesHeld-testset (40 vragen + rapporten); draai vóór/na elke botwijziging
...
- **Publieke URL staat op drie plekken**: `CANONICAL_ORIGIN` in `api/aanraders.mjs`, de `Sitemap:`-regel in `robots.txt` en `RECIPE_URL_PREFIX` in `api/chat.mjs` (receptlinks van HapjesHeld; ook `checkRecipeLinks()` matcht op dat domein) — bij een domeinwijziging alle drie. Nu `community-web.prilleven.be`; de bedoeling is `community.prilleven.be`, maar daar draait nog **Kollab** (versie 1 van de community, extern platform via `clientportal.ludicrous.cloud`). Die naam pas overnemen bij de migratie van v1 naar deze app, niet eerder. `/robots.txt` is statisch, `/sitemap.xml` loopt via een rewrite naar `/api/aanraders?sitemap=1`.
...
- **Betalingen kwamen tussen 13-08 en 02-09-2026 niet door:** Plug&Pay stuurde JSON met content-type `x-www-form-urlencoded`, waardoor de webhook-parser het e-mailadres niet vond. 127 events niet verwerkt, 87 leden buitengesloten, met de hand hersteld. Zie `api/CLAUDE.md` voor de fix en de monitoring-query. Een stille webhook-fout kost weken voor je hem ziet — controleer `subscription_events` op `applied = false`. **Daarna, van 02-09 tot 20-09, opnieuw niets verwerkt**: `&dryrun=1` was na een test in de productie-URL van Plug&Pay blijven staan (123 klanten, hersteld op 23-09). En regel `504592` vuurt voor álle producten — sinds 23-09 laat een SKU-filter in de webhook alleen `119701` (Pril Leven Community) toegang geven.
