...
- **Publieke URL = `community.prilleven.be`** (niet `community-web`). Staat als `CANONICAL_ORIGIN` in `api/aanraders.mjs` en in `robots.txt`; bij domeinwijziging beide aanpassen. `/robots.txt` is statisch, `/sitemap.xml` wordt gegenereerd via een rewrite naar `/api/aanraders?sitemap=1`.
