...
import * as Store from './js/store.js?v=2.29.0';
import {
  checkAllowedUser,
  checkCanSignUp,
  authSignUp,
...
} from './js/supabase.js?v=2.29.0';
import * as Router from './js/router.js?v=2.29.0';
import * as Header from './js/components/header.js?v=2.29.0';
import * as Nav from './js/components/nav.js?v=2.29.0';
import * as Home from './js/components/home.js?v=2.29.0';
...
// onze grootste sub-app (eersteHapjes).
...
async function initApp() {
  ...
  // Pre-load admin status uit het reeds opgehaalde status-object
  // (geen tweede fetch — bespaart ~150-300ms op page-load).
  await Store.refreshAdminStatus({ fromStatus: status });
...
function showSubscriptionExpiredScreen(status) {
  ...
  document.getElementById('sub-refresh-btn').addEventListener('click', async () => {
    ...
    if (fresh.active) {
      overlay.remove();
      await Store.refreshAdminStatus({ fromStatus: fresh });
      setupApp();
    } else {
      btn.disabled = false;
...
function showAuthModal(initialView = 'login') {
  ...
  async function completeLogin(email) {
    ...
    // Pre-load admin uit reeds opgehaalde status (geen extra fetch).
    await Store.refreshAdminStatus({ fromStatus: status });
    setupApp();
  }
...
function setupApp() {
  ...
  Router.on('recipes', async () => {
    const RecipeList = await import('./js/components/recipeList.js?v=2.29.0');
    await renderPage(RecipeList.render(), RecipeList.init);
  });

  /* --- Recept bewerken --- */
  Router.on('edit/:id', async (params) => {
    const RecipeForm = await import('./js/components/recipeForm.js?v=2.29.0');
    await renderPage(
  ...
  Router.on('recipe/:id', async (params) => {
    const RecipeDetail = await import('./js/components/recipeDetail.js?v=2.29.0');
    await renderPage(
  ...
  Router.on('eerste-hapjes', async () => {
    const EersteHapjes = await import('./js/components/eersteHapjes.js?v=2.29.0');
