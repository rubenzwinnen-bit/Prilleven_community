...
- **Let op — vulkleur vs. tekstkleur.** `--color-primary` (#C98966, terracotta) is de hoofdkleur; `--color-secondary`/`-dark` (#98C3A4 / #82BE93) zijn salie-groene **vulkleuren** voor achtergronden, bollen en badges. Op de paginakleur #faf8f5 halen ze slechts 1.9 en 2.0 contrast — te weinig voor tekst (WCAG: 4.5 gewoon, 3.0 grote koppen). Gebruik voor **alle groene tekst** `--color-green-text` (#4F7D6C merkgroen, contrast 4.42 op #faf8f5 en 4.68 op wit). Sinds V3.2 een variabele in `:root`; alle voorheen hardcoded #4a7c59 in styles.css zijn erop omgezet. In gebruik door o.a. `.header-title` ("Community Pril leven"), `.btn-logout` ("Uitloggen"), de recept- en weekschema-tags en de hele /aanraders-pagina. Eén plek wijzigen = overal door.
...
- **Valkuil — meerdere module-instanties.** `supabase.js` wordt in de app onder ~8 verschillende `?v=`-strings geïmporteerd; elke string is voor de browser een apart bestand, dus er draaien meerdere kopieën naast elkaar. De sessie zelf zit in localStorage en werkt daardoor, maar de dedup van gelijktijdige token-refreshes werkt alleen bínnen één kopie. Nieuwe modules: sluit aan bij de meest gebruikte versie (nu `2.5.10`) in plaats van een nieuwe toe te voegen.
