...
| `chat.js` | Logic voor `chat.html` — chat-interface met sidebar (conversations) + memory-modal. Onder elk AI-antwoord 👍/👎 via `attachFeedback()` → `/api/chat-feedback` (bij 👎 optioneel een reden; nogmaals klikken = ongedaan). Vervangt sinds 2026-09-26 "Dit helpt mij" uit `localStorage`. Stuurt `stream: true` naar `/api/chat` en leest de SSE via `readChatStream()`; meescrollen enkel zolang `followBottom` (wie omhoog scrolt, zet het uit; terug onderaan zet het aan). Bij `done` wordt de bubbel ter plaatse vervangen door het gecontroleerde antwoord (`checkRecipeLinks` server-side). Daarna leest `readSuggestions()` nog het `suggestions`-event: knopjes (`.chat-suggestions`) onder het laatste antwoord, klik = meteen versturen; ze verdwijnen bij een nieuwe vraag en laat binnenkomende worden genegeerd (`sendSeq`). Profile-modal is verwijderd (V2.8.0); memory-toggle + GDPR staan nu op `/profiel`. `loadProfile()` blijft voor de quota-bar (consumeert `data.usage` uit `/api/profile`). |
| `admin-chat.js` | Logic voor `admin-chat.html` — admin dashboard tabs (overview, users, queries, fallbacks, feedback, events, chat). Bevat `authedFetch` (GET) + `authedPost` (POST) helpers. Chat-tab laadt lazy de reports queue via `loadChatReports()` + `initChatTab()` (event delegation op `#chat-reports-list`). |
