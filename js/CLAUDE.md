...
| `learningsLibrary.js`, `learningsDetail.js`, `learningProgress.js` | Learnings/documentenzone met `Mijn leerpad`: de lijst-API levert bestaande bookmarks mee, kaarten tonen `Nieuw`/`Bezig`/`Afgerond`, de samenvatting kiest de recentste bookmark voor `Ga verder met…` en elk detail heeft dezelfde bewuste afrond-/undoactie. `Afgerond` is voorlopig een lokale preview per gebruiker (`prilleven_learning_progress_<user>`); bookmarks blijven de bestaande serverdata en alleen die maken een item `Bezig`. |
