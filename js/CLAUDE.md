...
| `weekSchedule.js` | Weekschema (5 slots × 7 dagen). Het actieve schema gebruikt een compacte titel/filterbalk en per dag vijf visuele recepttegels; Cooked it staat als groen vinkje op de foto. `Pril Ritme` telt unieke kookdagen (doel: 3) en luistert naar de `storage`-event voor andere tabbladen. De generator kan favoriete recepten adaptief voorrang geven: allergenen/eetmoment blijven hard, dezelfde favoriet max. 2× bij voorkeur, en bij minder dan 4 niet-favorieten voor een eetmoment vervalt het onderscheid om een te kleine restpool niet te forceren. |
