...
| `home.js` | Landingspagina (hub). **Split-view layout** sinds 9-mei: tegels-pane (functies) + timeline-pane (community-feed) gerenderd naast elkaar. Op desktop (≥769px) grid `320px 1fr` — beide tegelijk zichtbaar. Op mobile (≤768px) `[data-active-pane]`-attribuut bepaalt welke pane zichtbaar is, geswitcht via sticky bottom-nav onderaan (Timeline ↔ Functies). Default = Timeline; keuze onthouden in `localStorage['home:active-pane']`. Tegels hebben `min-height: 96px` voor uniforme hoogte; hover-foto schuift uit met 100px (op mobile uit). Tegel-config in `tiles[]`-array met `hoverImg`-veld per tegel. |
...
| `eersteHapjes.js` | Eerste Hapjes-pagina (SPA-route `#/eerste-hapjes`). Laadt kindjes + logs (vandaag's meals + 7d meals + 7d symptoms + allergenen + intro-logs + fase-state + recipes-cache). Vandaag-layout: fase-banner → "Tips & herinneringen"-card → grid van cards (Maaltijden, Symptomen, Allergenen, Agenda, **info-tegels** Mijn fasen / Symptomen-uitleg / Risicovoedingen-lijst). Info-tegels gerenderd via `renderInfoCard(action, title, desc)`, klikbaar als `<div role="button">` met dezelfde data-action handlers die voorheen op de footer-pill-buttons zaten — eh-today-foot block weg. Suggestion-handlers: `open-intro`, `open-recipe` (Router.navigate), `open-meal-log`, `open-phase-detail`, `show-info` (toast). Exporteert `getActiveChildSnapshot()` + `loadActiveChild()` (gebruikt door `recipeDetail.js` voor risk-banner). |
...
| `articleModal.js` | Eerste Hapjes brok E + brok K — modal-shell met 2 weergaven: detail (één artikel met body als HTML) en lijst (gegroepeerd per categorie, "Voor jou nu"-pill bij relevante leeftijd). Lijst-view heeft search-input (matcht title + summary + body + categorie-label, met `<mark>`-highlight in titel) + categorie-chips (multi-select OFF, klik op zelfde chip = reset, klik op "Alle" = reset). Slug-navigatie via interne `swap(html)`. Modal-shell heeft `onBound`-callback waarmee renderList zijn search/chip-events kan binden na elke `swap`. |
...
| `agendaModal.js` | Eerste Hapjes — kalender-view (rewrite 9-mei). Twee views: **Week** (7 kolommen ma-zo, events stapelend per dag, kleuren-gecodeerd: groen=meal, oranje=symptom, bruin=intro, rood-rand bij `severe`) en **Maand** (7-koloms grid met `eh-cal-cell`-buttons, gekleurde dots per type, klik op cel → switcht naar week-view rond die dag). Navigatie: ◀ Vorige | range-label | Volgende ▶ + "Vandaag"-knop; cursor-state verspringt 7d (week) of 1m (maand). Eén initial fetch (90d historiek) ipv per range; cursor bepaalt welke subset gerenderd wordt. 3-maand-view weggehaald. Modal-breedte override (`max-width: min(960px, 96vw)`) want base `.modal` cap is 420px. Accepteert `initialData` voor instant week-view tot fetch klaar is. |
...
```js
import { supabaseFetch } from './supabase.js?v=2.22.0';
...
```js
import { sessionRefreshIfNeeded } from './supabase.js?v=2.22.0';
...
- **Alle HTML-bestanden** (`index.html`, `chat.html`, `admin-chat.html`, `delete-account.html`, `privacy.html`):
  ```html
  <script type="module" src="script.js?v=2.22.0"></script>
  <link rel="stylesheet" href="styles.css?v=2.22.0">
  ```
- **`script.js`** (entry point) — 14× in import statements:
  ```js
  import * as Store from './js/store.js?v=2.22.0';
  ```
- **Elke module in `/js`** die andere modules importeert — `store.js`, `chat.js`, `admin-chat.js`, `headerAvatarStandalone.js`, `communityApi.js`, en **alle** componenten in `js/components/*`. Voorbeeld uit `header.js`:
  ...
  import { sessionClear, sessionGet } from '../supabase.js?v=2.22.0';
  ```
...
### Hoe bumpen?
Vervang ALLE voorkomens van de huidige versie (bv. `?v=2.22.0`) met de nieuwe (bv. `?v=2.7.1`) in:
1. Alle HTML-bestanden in de root.
2. `script.js`.
3. Alle bestanden in `/js/*.js` en `/js/components/*.js` met imports.

Snelle check: `grep -rn "v=2.22.0" --include="*.js" --include="*.html"`.
