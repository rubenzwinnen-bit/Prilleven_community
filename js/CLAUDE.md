...
| `weekSchedule.js` | Weekschema (5 slots × 7 dagen). Het actieve schema toont de lokale Cooked it-status per slot en `Pril Ritme` op basis van unieke kookdagen (doel: 3). Luistert ook naar de `storage`-event zodat een in een ander tabblad afgerond gerecht zichtbaar wordt. De generator kan favoriete recepten adaptief voorrang geven: allergenen/eetmoment blijven hard, dezelfde favoriet max. 2× bij voorkeur, en bij minder dan 4 niet-favorieten voor een eetmoment vervalt het onderscheid om een te kleine restpool niet te forceren. |
