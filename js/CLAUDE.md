...
| `chat.js` | Logic voor `chat.html` — chat-interface met sidebar (conversations) + memory-modal. Onder elk AI-antwoord 👍/👎 via `attachFeedback()` → `/api/chat-feedback` (bij 👎 optioneel een reden; nogmaals klikken = ongedaan). Vervangt sinds 2026-09-26 "Dit helpt mij" uit `localStorage`. Profile-modal is verwijderd (V2.8.0); memory-toggle + GDPR staan nu op `/profiel`. `loadProfile()` blijft voor de quota-bar (consumeert `data.usage` uit `/api/profile`). |
