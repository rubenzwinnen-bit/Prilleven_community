...
### Chatfeedback (2026-09-26 — `2026-09-26-chat-feedback.sql`)
- **`chat_feedback`** — `id, message_id (FK messages, cascade), user_id (FK auth.users), rating ∈ (1,-1), reden (≤500), created_at, updated_at`, UNIQUE `(message_id, user_id)`. Geschreven door `/api/chat-feedback`; RLS enkel `SELECT` op eigen rijen.
