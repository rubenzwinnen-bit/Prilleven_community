...
- **`child_symptoms`** — `id, child_id (FK children cascade), user_id (FK auth.users cascade), occurred_at (default now), symptom_type ∈ ('huid','buik','diarree','braken','slaap','koorts','jeuk','zwelling','ademhaling','anders','gewicht','hoesten','verstopping','geen_eetlust','prikkelbaar','lethargie') (16 keys; brok G uitgebreid via `2026-05-09-symptoms-extend.sql` — additief, oude logs blijven geldig), severity ∈ ('mild','matig','heftig'), meal_log_id (FK meal_logs ON DELETE SET NULL — optionele koppeling), notes (≤500) NULL, created_at, updated_at`. Index `(child_id, occurred_at desc)`. updated_at trigger. RLS: 4 owner-only policies.
- **`child_allergens`** — `id, child_id (FK children cascade), user_id (FK auth.users cascade), allergen_key text (lowercase, 1-40 chars; vocabulaire-validatie gebeurt server-side, geen DB-constraint), status ∈ ('gepland','geprobeerd','vermijden'), reaction ∈ ('geen','mild','matig','heftig','onbekend') NULL, intro_date date NULL, notes (≤500) NULL, linked_symptom_id (FK child_symptoms ON DELETE SET NULL), created_at, updated_at`. Unique `(child_id, allergen_key)`. Index `(child_id)`. updated_at trigger. RLS: 4 owner-only policies. **Brok H.3 (zonder migratie)**: UI gebruikt `reaction`/`intro_date` in deze tabel niet meer — alle intro-events leven nu in `allergen_intro_logs`. `status` wordt alleen nog gebruikt voor `'gepland'` (default) of `'vermijden'` (handmatige vlag).
- **`allergen_intro_logs`** — `id, child_id (FK children cascade), user_id (FK auth.users cascade), allergen_key text (lowercase; vocabulaire server-side, geen DB-constraint), intro_date date (default current_date), reaction ∈ ('geen','mild','matig','heftig','onbekend') (default 'geen'), notes (≤500) NULL, meal_log_id (FK meal_logs ON DELETE SET NULL), linked_symptom_id (FK child_symptoms ON DELETE SET NULL), created_at, updated_at`. Indexen `(child_id, allergen_key, intro_date desc)` + `(user_id, intro_date desc)`. updated_at trigger (`touch_updated_at`). RLS: 4 owner-only policies. **Brok H.2** — eén rij per intro-poging (basis voor 1/3-2/3-3/3 voortgang + tijdlijn).
- **`child_phases`** — `child_id (FK children cascade), user_id (FK auth.users cascade), phase_number smallint (0..5), unlocked_at (default now), completed_at, created_at, updated_at`. PK `(child_id, phase_number)`. Index `(child_id)`. updated_at trigger. RLS: 4 owner-only policies. State-only — fase-definities staan in `js/content/eersteHapjes-phases.js`. Auto-init bij eerste GET: ≥14 mnd → fases 0..4 'completed' + fase 5 actief; anders → enkel fase 0 actief.
- **`child_phase_checks`** — `child_id (FK children cascade), user_id (FK auth.users cascade), phase_number smallint (0..5), check_key text (lowercase, 1-60 chars), checked_at (default now)`. PK `(child_id, phase_number, check_key)`. Index `(child_id, phase_number)`. RLS: 3 owner-only policies (read/insert/delete — geen update; toggle = insert of delete).
